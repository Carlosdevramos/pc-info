...
async function fetchInfo() {
  ...
  if (!res.ok) throw new Error("Erro ao buscar info: " + res.statusText);
  const data = await res.json();
  return data;
}

function humanBytes(n: number) {
  if (!n && n !== 0) return "Desconhecido";
...
function render(data: any) {
  ...
  data.memory.layout.forEach((m: MemModule, idx: number) => {
  ...
  data.filesystems.forEach((f: any) => {
    const item = document.createElement("div");
    item.className = "kv";
...
async function loadAndRender() {
  ...
  try {
    ...
    (document.getElementById("content") as HTMLDivElement).innerHTML = `<pre>${String(e)}</pre>`;
  } finally {
    btn.disabled = false;
